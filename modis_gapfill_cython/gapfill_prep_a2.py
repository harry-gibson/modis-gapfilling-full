...
from gapfill_config import FlagValues, A2SearchConfig, DataSpecificConfig

def A2Caller(dataImageIn, flagsImageIn, distImageIn, meanImageIn):
    ...
    _A2_MAX_NBRS = A2SearchConfig["MAX_NBRS_TO_SEARCH"]
    ...
    # TODO if we can afford the memory it'd be much faster to make a copy then transpose it back at the end
    ...
    for x in range(len(xCorners)-1):
        for y in range(len(yCorners)-1):
            ...
            if A2SearchConfig["PASS_SELECTOR_MEAN_OR_MEDIAN"] == "MEAN":
    ...
    print ("A2 done, filled {0} locations of {1} required in {2} seconds using {3} of 8 passes".format(
                                                    ...
                                                    A2SearchConfig["PASS_SELECTOR_MEAN_OR_MEDIAN"]))
    # we don't return anything, the arrays have been modified in-place
